...
import { rpSchema } from "@/schemas/presupuesto.schema";
import { crearRP } from "@/services/presupuesto.service";
...
export async function GET(request: NextRequest) {
  ...
  const [rps, total] = await Promise.all([
    ...
    prisma.rP.count({ where }),
  ]);

  return NextResponse.json({ data: rps, total, page, limit });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    verificarPermiso(session.user.rol, "rp:crear");
    const body = await request.json();
    const data = rpSchema.parse(body);
    const rp = await crearRP(data, session.user.id);
